  ...
  if (error)
    return (
      <div className="p-6 glass rounded-xl bg-red-500/10 text-red-400 flex items-center animate-fade-in">
        <AlertTriangle className="mr-2" size={20} /> Error: {error}
      </div>
    );
  ...
  return (
    <div className="space-y-4">
      <div className="mb-2 flex items-center gap-3">
        ...
        <h4 className="text-lg font-bold bg-gradient-to-r from-purple-500 to-blue-500 bg-clip-text text-transparent">
      ...
      {Array.isArray(insights) && insights.length > 0 ? (
        (insights || []).map(insight => (
          <div
            key={insight.id}
            className="p-4 glass rounded-xl shadow-md bg-gradient-to-r from-purple-400/10 to-blue-400/10 animate-fade-in"
          >
